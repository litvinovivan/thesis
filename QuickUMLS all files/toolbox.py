# Modified version to be based around SNOMED CT concept IDs instead of UMLS cuis - Ivan
...
#changed the above to this line - IL
import simstring
...
class CuiSemTypesDB(object):
    ...
    def insert(self, term, code, cui, semtypes, is_preferred):
        term = prepare_string_for_db_input(safe_unicode(term))
        cui = prepare_string_for_db_input(safe_unicode(cui))
        code = prepare_string_for_db_input(safe_unicode(code))

        # some terms have multiple cuis associated with them,
        # so we store them all
        try:
            codes = pickle.loads(self.cui_db.Get(db_key_encode(term)))
        except KeyError:
            codes = set()

        codes.add((code, cui, is_preferred))
        self.cui_db.Put(db_key_encode(term), pickle.dumps(codes))
    ...
    def get(self, term):
        ...
        matches = (
            ...
            for code, cui, is_preferred in codes
        )
        return matches
